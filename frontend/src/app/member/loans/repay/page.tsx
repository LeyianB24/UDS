...
function RepayContent() {
    ...
    if (loading || !loan) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                <div className="w-12 h-12 border-4 border-[#0b2419]/10 border-t-emerald-500 rounded-full animate-spin"></div>
    ...
    return (
        <div className="pb-20">
            {/* Header / Hero Section */}
            <div className="relative overflow-hidden bg-[#0b2419] rounded-b-[48px] pt-12 pb-24 px-6 md:px-12 text-white">
                <div className="absolute top-0 right-0 p-8 opacity-10 pointer-events-none">
                    <Wallet2 size={320} className="md:translate-x-32 md:-translate-y-32" />
                </div>
                
                <div className="max-w-6xl mx-auto relative z-10">
                    <Link href="/member/loans" className="group inline-flex items-center gap-2 text-lime text-[10px] font-black uppercase tracking-[0.2em] mb-8 hover:text-white transition-all">
                        <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> Back to Dashboard
                    </Link>
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
                        <div>
                           <div className="flex items-center gap-3 mb-2">
                               <span className="px-3 py-1 bg-lime/10 border border-lime/20 rounded-full text-[9px] font-black text-lime uppercase tracking-widest">Loan ID #{loan.loan_id}</span>
                        ...
                        <div className="text-right">
                           <div className="text-[10px] font-black text-lime/50 uppercase tracking-[0.3em] mb-2">Outstanding Balance</div>
                           <div className="text-4xl md:text-6xl font-black tracking-tightest">
                               <span className="text-xl md:text-2xl text-lime mr-2 font-bold uppercase">KES</span>
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        <div className="bg-white/80 backdrop-blur-xl border border-white rounded-[40px] p-8 shadow-[0_32px_64px_-16px_rgba(11,36,25,0.08)]">
                            <h3 className="text-xs font-black text-[#0b2419] uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                                <CreditCard size={14} className="text-emerald-600" /> Loan Integrity
                            </h3>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between items-center py-4 border-b border-slate-50">
                                    ...
                                    <span className="text-sm font-black text-emerald-600 uppercase tracking-tight">{loan.interest_rate}% Annually</span>
                                </div>
                                <div className="flex justify-between items-center py-4 border-b border-slate-50">
                            ...
                            <div className="mt-8 p-6 bg-[#0b2419] rounded-3xl relative overflow-hidden group hover:shadow-xl transition-all">
                                ...
                                <p className="text-[10px] font-bold text-lime/70 leading-relaxed relative z-10">
                        ...
                        <div className="bg-white/90 backdrop-blur-xl border border-white rounded-[40px] p-8 md:p-12 shadow-[0_32px_96px_-24px_rgba(11,36,25,0.12)]">
                            ...
                            <div className="flex items-center gap-4 mb-10 pb-6 border-b border-slate-50">
                                <div className="w-16 h-16 bg-[#39B54A] rounded-2xl flex items-center justify-center text-white shadow-[0_12px_24px_-4px_rgba(57,181,74,0.3)]">
                                    <img src="/assets/images/mpesa_logo.png" alt="M-Pesa" className="w-10 h-auto brightness-200" />
                                </div>
                                <div>
                            ...
                            <form onSubmit={handleRepay} className="space-y-8">
                                ...
                                <button 
                                    ...
                                    className="w-full h-20 bg-[#0b2419] text-lime rounded-[28px] font-black text-lg uppercase tracking-[0.3em] flex items-center justify-center gap-4 hover:shadow-[0_20px_40px_-8px_rgba(11,36,25,0.3)] hover:-translate-y-1 transition-all active:scale-95 disabled:opacity-50 disabled:translate-y-0 disabled:shadow-none"
                                >
                                    {submitting ? (
                                        <span className="w-6 h-6 border-4 border-lime/20 border-t-lime rounded-full animate-spin"></span>
                                    ) : (
                                        <>
                                            <Send size={24} />
                                ...
                                <div className="text-center">
                                   <Link href="/member/loans" className="text-[10px] font-black text-slate-300 uppercase tracking-widest hover:text-[#0b2419] transition-colors">
            ...
            <style jsx>{`
                .tracking-tightest { letter-spacing: -3.5px; }
                @media (max-width: 768px) {
                    .tracking-tightest { letter-spacing: -1.5px; }
                }
            `}</style>
        </div>
    );
...
export default function RepayLoanPage() {
    return (
        <Suspense fallback={
            <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
                <div className="w-12 h-12 border-4 border-[#0b2419]/10 border-t-emerald-500 rounded-full animate-spin"></div>
                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Booting secure framework...</p>
            </div>
        }>
            <RepayContent />
        </Suspense>
