...
export default function AdminLedgerPage() {
  ...
  const health = (data as Record<string, Record<string, unknown>> | null)?.health;
  const logs = (data as Record<string, unknown[]> | null)?.logs;
    ...
    <div className="space-y-8 pb-20">
      ...
      <div className="bg-gradient-to-br from-[#0f2e25] to-[#1a5c42] text-white rounded-[32px] p-8 lg:p-12 relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-8 shadow-xl shadow-green-950/20">
         ...
         <div className="relative z-10 flex flex-col gap-3 shrink-0">
             <button onClick={loadData} title="Refresh Feed" className="px-6 py-4 bg-lime-400 text-[#0f2e25] font-black rounded-2xl text-xs uppercase tracking-widest hover:bg-lime-300 transition-colors flex items-center justify-center gap-2 shadow-lg shadow-lime-400/20">
      ...
      <div className="flex flex-wrap gap-2 bg-white border border-gray-200 rounded-2xl p-2 w-fit shadow-sm">
          <button onClick={() => setActiveTab('feed')} title="Operations Feed" className={cn("px-6 py-3 rounded-xl text-sm font-bold transition-all flex items-center gap-2", activeTab === 'feed' ? "bg-[var(--brand-forest)] text-white shadow-md" : "text-gray-500 hover:text-gray-900")}>
              <Activity size={16} /> Operations Feed
          </button>
          <button onClick={() => setActiveTab('health')} title="Health & Integrity" className={cn("px-6 py-3 rounded-xl text-sm font-bold transition-all flex items-center gap-2", activeTab === 'health' ? "bg-[var(--brand-forest)] text-white shadow-md" : "text-gray-500 hover:text-gray-900")}>
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm relative overflow-hidden group hover:-translate-y-1 transition-transform">
                  ...
                  <div className="text-3xl font-black text-[var(--brand-forest)]">{String(health?.callback_success_rate ?? 0)}%</div>
                  <div className="mt-4 h-1 w-full bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full bg-green-500 rounded-full" style={{ width: `${health?.callback_success_rate ?? 0}%` }}></div>
                  </div>
              ...
              <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm relative overflow-hidden group hover:-translate-y-1 transition-transform">
                  ...
                  <div className="text-3xl font-black text-amber-600">{String(health?.pending_transactions ?? 0)}</div>
              ...
              <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm relative overflow-hidden group hover:-translate-y-1 transition-transform">
                  ...
                  <div className="text-3xl font-black text-red-600">{String(health?.failed_notifications ?? 0)}</div>
              ...
              <div className="bg-gradient-to-br from-[var(--brand-forest)] to-[#1a5c42] rounded-3xl p-6 shadow-sm relative overflow-hidden group hover:-translate-y-1 transition-transform border-none">
                  ...
                  <div className="text-2xl font-black text-white">{formatKES(Number(health?.daily_volume ?? 0))}</div>
                                ...
                                return (
                                  <tr key={l.id} className="hover:bg-gray-50/50">
                                      <td className="px-6 py-4">
                                          <div className="text-xs font-black text-gray-900 font-mono">{new Date(l.created_at).toLocaleTimeString()}</div>
                                          <div className="text-[10px] font-bold text-gray-500 mt-0.5">{new Date(l.created_at).toLocaleDateString()}</div>
                                      </td>
                                      <td className="px-6 py-4">
                                          <div className="text-xs font-black text-gray-900">{l.action}</div>
                                          <div className="text-[10px] font-bold text-gray-500 mt-0.5">{l.user_type || 'System'}</div>
                                      </td>
                                      <td className="px-6 py-4">
                                          <span className={cn(
                                              "px-2 py-1 rounded border text-[10px] font-black uppercase tracking-widest",
                                              l.severity === 'info' ? "bg-blue-50 text-blue-600 border-blue-200" :
                                              l.severity === 'warning' ? "bg-amber-50 text-amber-600 border-amber-200" :
                                              l.severity === 'error' ? "bg-red-50 text-red-600 border-red-200" :
                                              "bg-green-50 text-green-600 border-green-200"
                                          )}>{l.severity || 'INFO'}</span>
                                      </td>
                                      <td className="px-6 py-4 text-xs font-bold text-gray-600 max-w-[300px] truncate">
                                          {l.details}
                                      </td>
                                      <td className="px-6 py-4 text-right">
                                          <span className="px-2 py-1 bg-gray-100 text-gray-500 rounded font-mono text-[10px] font-bold">{l.ip_address}</span>
                                      </td>
                                  </tr>
                                );
                              })
      ...
      <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm relative overflow-hidden">
                  <div className="flex items-start justify-between mb-2">
                      ...
                      <span className={cn("w-2 h-2 rounded-full", health?.ledger_imbalance ? "bg-red-500 animate-pulse" : "bg-green-500")}></span>
                  </div>
                  ...
                  <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                      <span className={cn("px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest", health?.ledger_imbalance ? "bg-red-50 text-red-600" : "bg-green-50 text-green-600")}>
                          {health?.ledger_imbalance ? 'Imbalance Detected' : 'Healthy'}
                      </span>
                  </div>
                  <div className={cn("absolute bottom-0 left-0 w-full h-1", health?.ledger_imbalance ? "bg-red-500" : "bg-green-500")} />
              </div>
              ...
              <div className="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm relative overflow-hidden">
                  ...
                  <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                      <span className="text-xl font-black text-[var(--brand-forest)]">{String(health?.db_size ?? 0)} MB</span>
          ...
          <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  <button onClick={() => handleSystemAction('test_connectivity')} title="Test API Connectivity" className="p-6 bg-gray-50 hover:bg-white border-2 border-transparent hover:border-gray-200 rounded-2xl text-center transition-all group shadow-sm hover:shadow-md">
                  ...
                  <button onClick={() => handleSystemAction('clear_cache')} title="Purge Cache" className="p-6 bg-gray-50 hover:bg-white border-2 border-transparent hover:border-gray-200 rounded-2xl text-center transition-all group shadow-sm hover:shadow-md">
                  ...
                  <button onClick={() => handleSystemAction('resync_financials')} title="Resync Ledgers" className="p-6 bg-gray-50 hover:bg-white border-2 border-transparent hover:border-gray-200 rounded-2xl text-center transition-all group shadow-sm hover:shadow-md">
                  ...
                  <button onClick={() => handleSystemAction('run_audit')} title="Run Deep Audit" className="p-6 bg-[var(--brand-forest)] text-lime-400 hover:bg-green-900 rounded-2xl text-center transition-all group shadow-sm hover:shadow-md">
