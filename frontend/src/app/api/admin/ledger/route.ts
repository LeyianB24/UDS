...
export async function GET(request: Request) {
    try {
        ...
        // 1. Health Stats
        const callbackSuccessRate = 95; // Mock data since we don't have actual callbacks recorded
        const [pendingStkRows] = await pool.execute<RowDataPacket[]>(
            `SELECT COUNT(*) as c FROM transactions WHERE payment_channel='mpesa' AND status='pending'`
        );
        const pendingStk = pendingStkRows[0].c;

        const [failedCommsRows] = await pool.execute<RowDataPacket[]>(
            `SELECT COUNT(*) as c FROM notifications WHERE is_read=0 AND created_at >= DATE_SUB(NOW(), INTERVAL 1 DAY)`
        );
        const failedComms = failedCommsRows[0].c;

        const [dailyVolumeRows] = await pool.execute<RowDataPacket[]>(
            `SELECT SUM(amount) as v FROM transactions WHERE DATE(created_at) = CURDATE()`
        );
